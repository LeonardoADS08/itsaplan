...
// What the agent may do: the team role its requests act under, which applies in every
// project it works in, and — for an external agent — whose runs its runner is served.
export default function AgentAccessSection({
  ...
  // The form seeds the team's default role once the list loads, so a null value only
  // means the roles are not there yet.
  const selectedRoleValue = value.roleId != null ? String(value.roleId) : NO_ROLE_VALUE;
